...
class Subscriptions(commands.Cog):
    ...
    async def ainit(self):
        async with aiopg.connect(sets["psqlUri"]) as conn:
            async with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                await cur.execute("SELECT id, disabled, server_id FROM channel")
        ...
        for usr in usrs:
            usr_obj = self.repcog.User(usr['id'], usr['token'])
            msg += f'{usr_obj}\n'
        msg += ' {ttt}'
        del usrs
        ...
        for wall in walls:
            wall_obj = self.repcog.Wall(wall['id'], wall['public'], wall['last_id'])
            msg += f'{wall_obj}\n'
        msg += ' {ttt}'
        del walls
        self.logger.info(msg)
        ...
        for srv in srvs:
            srv_obj = self.repcog.Server(srv['id'], srv['lang'])
            msg += f'{srv_obj}\n'
            for chn in chns:
                if chn['server_id'] == srv_obj.id:
                    chn_obj = self.repcog.Server.Channel(srv_obj, chn['id'], chn['disabled'])
                    msg += f'\t{chn_obj}\n'
                    for sub in subs:
                        if sub['channel_id'] == chn_obj.id:
                            usr_obj = self.repcog.User.find_by_args(sub['users_id'])
                            wall_obj = self.repcog.Wall.find_by_args(sub['wall_id'])
                            sub_obj = self.repcog.Server.Channel.Subscription(chn_obj, wall_obj, usr_obj, sub['msg'], sub['id'])
                            msg += f'\t\t{sub_obj}\n'
        msg += ' {ttt}'
        del srvs, chns, subs
        self.logger.info(msg)
    ...
    @commands.bot_has_permissions(send_messages=True, embed_links=True, manage_messages=True, read_message_history=True)
    ...
    async def sub_add(self, ctx: SlashContext, wall_id: str, channel: discord.TextChannel=None):
        ...
        # Req for walls
        reqs.append({'wall_REQ': vk_pool.add_call('groups.getById', ctx.bot_usr.token, self.groups_getById_attrs | {'group_id': wall_id}), 'ERR': None, 'type': 'grp'})
        reqs.append({'wall_REQ': vk_pool.add_call('users.get', ctx.bot_usr.token, self.users_get_attrs | {'user_ids': wall_id}), 'ERR': None, 'type': 'usr'})
        ...
        #Wrap
                wrapped = VKRespWrapper(req['type'], req['wall_RESP'][0], req['post_RESP'], vk_info=self.client.vk_info)
                if wrapped.type == 'grp':
                    grp = wrapped
                else:
                    usr = wrapped
            else:
                wrapped = VKRespWrapper(req['type'], error=req['ERR'])
                if wrapped.type == 'grp':
                    grp = wrapped
                else:
                    usr = wrapped
        ...
        if (not grp.error) and (not usr.error):
            ...
            ctx.msg = await ctx.send(content='Select the wall', embeds=[grp.wall_embed, usr.wall_embed], components=[action_row])
        ...
        action_row = create_actionrow(*buttons)
        if hasattr(ctx, 'msg'):
            await ctx.msg.edit(content='Is this the wall you requested?', embed=wrapped.wall_embed, components=[action_row])
        else:
            ctx.msg = await ctx.send(content='Is this the wall you requested?', embed=wrapped.wall_embed, components=[action_row])
        button = await wait_for_component(self.client, components=action_row, check=lambda btn_ctx: btn_ctx.author_id == ctx.author_id, timeout=120.0)
        await button.defer(edit_origin=True)
        ...
        if button.custom_id == 'yes':
            ...
            if len(subs) == 0:
                ...
                if wall is None:
                    wall, wallmsg = await self.repcog.Wall_add(wrapped.id, wrapped.post['items'][0]['id'])
                ...
                sub_embed = wrapped.wall_embed.copy()
                sub_embed.add_field(
                    name='Added by',
        ...
        if not chn.disabled:
            ...
            async with AsyncVkExecuteRequestPool() as vk_pool:
                for sub in chn.subscriptions:
                    if sub.wall.id < 0:
                        walls.append({'wall_REQ': vk_pool.add_call('groups.getById', ctx.bot_usr.token, self.groups_getById_attrs | {'group_id': abs(sub.wall.id)}), 'type': 'grp'})
                    else:
                        walls.append({'wall_REQ': vk_pool.add_call('users.get', ctx.bot_usr.token, self.users_get_attrs | {'user_ids': sub.wall.id}), 'type': 'usr'})
            for wall in walls:
                wall_REQ = wall['wall_REQ']
                if wall_REQ.ok:
                    wall['wrapped'] = VKRespWrapper(wall['type'], wall_REQ.result[0], vk_info=self.client.vk_info)
            ...
            ctx.msg = await ctx.send(content='Select the Subscription you want to manage', components=[action_row])
            ...
            sub_embed = discord.Embed.from_dict(deepcopy(wrapped.wall_embed.to_dict()))
            sub_embed.add_field(
                name='Added by',
                value=self.client.get_user(sub.user.id).mention,
                inline=False)
            ...
            try:
                ...
                if button.custom_id == 'change':
                    ...
                    await ctx.msg.edit(content=f'🗑️ Successfully unsubscribed {channel.mention} from this Wall!', embeds=[wrapped.wall_embed], components=[])
                    logmsg += f'{wallmsg}\n'
                    self.logger.info('Del {aa}SUB{aa} {tttpy}\n{msg} {ttt}'.format_map(SafeDict(msg=logmsg)))
        else:
            buttons = [create_button(style=ButtonStyle.green, label='Enable', custom_id='enable')]
            action_row = create_actionrow(*buttons)
            ctx.msg = await ctx.send(content=f'Channel Subscriptions are disabled.', embeds=[], components=[action_row])

            button = await wait_for_component(self.client, components=action_row, check=lambda btn_ctx: btn_ctx.author_id == ctx.author_id, timeout=120.0)
            if button.custom_id == 'enable':
                ...
                if missing_perms:
                    raise SubscriptionChannelMissingPermissions(missing_perms, channel)
                else:
                    chn.disabled = False
                    await ctx.msg.edit(content=f'✅ Successfully enabled Channel Subscriptions in {channel.mention}!', embeds=[], components=[])
                    
    async def initial_setup(self, ctx: SlashContext, channel: discord.TextChannel=None):
        ctx.bot_USR, usrmsg = self.repcog.User.find_by_args(ctx.author.id)
        if ctx.bot_USR is None:
            ctx.bot_USR, usrmsg = await self.repcog.User_add(ctx.author.id)
        if ctx.bot_USR.token is None:
    ...
    @cog_ext.cog_subcommand(base='subs', name='account',
                            ...
                            guild_ids=None if sets['version'] == 'MAIN' else [sets['srvcSrv']])
    @commands.bot_has_permissions(send_messages=True, embed_links=True, add_reactions=True, manage_messages=True, read_message_history=True)
    async def account(self, ctx: SlashContext):
        ...
        wrapped = VKRespWrapper(wall=usr_RESP[0], vk_info=self.client.vk_info)

        buttons = [create_button(style=ButtonStyle.blue, label='Relogin')]
        action_row = create_actionrow(*buttons)
        ctx.msg = await ctx.send(content='You are logged in with this account', embed=wrapped.wall_embed, components=[action_row])
        try:
            button = await wait_for_component(self.client, components=action_row, check=lambda btn_ctx: btn_ctx.author_id == ctx.author_id, timeout=120.0)
        except TimeoutError:
            buttons = [create_button(style=ButtonStyle.blue, label='Relogin (timeout)', disabled=True)]
            action_row = create_actionrow(*buttons)
            await ctx.msg.edit(content='You are logged in with this account', embed=wrapped.wall_embed, components=[action_row])
        else:
            await button.defer(edit_origin=True)
            await self.login(ctx)
    ...
    async def authentication(self, data):
        ...
        await msg.edit(content="You are now logged in with this account", embed=wrapped.wall_embed, components=[])
